...
service.onMessageReceived = { message in
    ...
    if let action = try? JSONDecoder().decode(DeviceAction.self, from: message.payload) {
        ...
        if let response = action.command.shell(arguments: action.params) {
            try! service.publish(topic: "raspberry.\(device.macAddress).out", payload: response).wait()
        }
    }
}

defer {
    try? service.unsubscribe(topic: "raspberry.\(device.macAddress).in").wait()
    try? service.disconnect().wait()
    try? eventLoopGroup.syncShutdownGracefully()
}

try service.connect().wait()
try service.subscribe(topic: "raspberry.\(device.macAddress).in", qos: .atLeastOnce).wait()
